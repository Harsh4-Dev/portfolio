/* history.js — the ML-history page for the terminal theme: eras on the left, the diagram rendered as ASCII on the right. */
import { loadContent, esc } from './content.js';
import { AsciiScreen } from './ascii.js';
...
async function main() {
  ...
  const show = (i, manual) => {
    ...
    scene?.setShape(eras[cur].shape || 'constellation');
    if (!reduced) scramble(items[cur].querySelector('.y'));
    if (manual) { clearInterval(timer); timer = setInterval(() => show(cur + 1), 9000); }
  };
  ...
  timer = setInterval(() => show(cur + 1), 9000);
}
main();
