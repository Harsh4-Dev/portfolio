...
def write_theme_boot(content: dict, site_dir: Path) -> Path | None:
    """Write assets/js/theme-boot.js so the first paint already uses the right background.

    Generated (never hand-edited): it carries the default theme and whether visitors may switch,
    so a terminal-themed site does not flash cream while content.json is still loading.
    """
    S = content["settings"]
    pl = plan(content)
    bg = str(S.get("terminal_bg") or "#000000")
    js = (
        "/* generated by tools/build_content.py - do not edit; sets the background before first paint */\n"
        "(function () {\n"
        "  try {\n"
        f"    var def = {json.dumps(pl['theme'])}, locked = {'false' if pl['theme_toggle'] else 'true'}, pick = def;\n"
        "    if (!locked) {\n"
        ...
        "    }\n"
        "    if (pick === 'terminal') {\n"
        f"      document.documentElement.style.background = {json.dumps(bg)};\n"
        "      document.documentElement.classList.add('pre-terminal');\n"
        "    }\n"
        "  } catch (e) { /* never block the page */ }\n"
        "})();\n"
    )
    target = site_dir / "assets" / "js" / "theme-boot.js"
...
def write(content: dict, out: Path = DEFAULT_JSON) -> Path:
    ...
    write_theme_boot(content, out.parent)
    return out
